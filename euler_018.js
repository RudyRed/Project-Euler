...
const maxSumPath = function (pyramid) {

  for (let rowIndex = 1; rowIndex < pyramid.length; rowIndex++) {
    let row = pyramid[rowIndex];
    let lastRow = pyramid[rowIndex -1];
    ...
    for(let i = 0; i < row.length; i++) {
      if (i === 0) {
        ...
        row[i] += Math.max(lastRow[i - 1], lastRow[i]);
      }
    }
  }
  
  return pyramid[pyramid.length - 1].reduce((a, i) => Math.max(a, i));
};
